...
import { IHouse, ISight } from '../../../models'
import { setNotification } from '../../../store/slices/notificationSlice'
import { setIsLoading } from '../../../store/slices/userSlice'

import './style.css'

interface ISightUpdateRequest extends ISight {
...
const EditSight: FC = () => {
	...
	const onUpload = async (e: any) => {
		...
		if (isValidName && isValidLocation && isValidnewCategory) {
			...
			try {
				await api
					.put<ISightUpdateRequest>(`/sights/${sight.sightId}`, formData)
					.then(response => {
						dispatch(
							setNotification({
								message: 'Достопримечательность успешно отредактировано',
								isError: false,
								errors: [],
							})
						)
